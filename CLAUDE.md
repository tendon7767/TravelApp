...
- **搜尋 grounding 開著**（`PLACE_TOOLS`）。免費額度是實際用量的幾十倍，而且沒綁付款方式
  就不可能被扣款 —— 免費層若不給用會回錯誤，不會偷偷收費。
  **搜尋工具與結構化輸出併用只有 Gemini 3 以後支援**，2.5 與更舊的會直接被拒，換模型要一起確認。
  開著搜尋時模型偶爾會在 JSON 前後多吐文字，所以後端的 `parsePlaceJson` 會退而求其次
  從第一個 `{` 撈到最後一個 `}`。一次要跑十幾二十秒，前端逾時因此放寬到 60 秒。
