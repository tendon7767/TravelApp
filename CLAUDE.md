...
- **選擇支付方式的選單是「這一筆用哪張刷」的排行榜，不是清單。** 逐條列出各規則的邊際回饋、依本幣（`inHomeCurrency`）由高到低排，各持有人分區**各自排名、各自掛一個「推薦」**——跨持有人排名會推薦到別人的卡。分區用 `.seg` 切換、一次只顯示一區（只有一位持有人時整條不出現），預設落在「目前那張卡的持有人」→「自己的卡」→ 第一區；切區要把 `.sheetbody` 捲回頂端。**現金／其他／未設定不歸任何持有人，固定釘在最底**，不隨切換消失。**推薦差距不到 1 元（本幣）就不掛**，為了三毛錢掛推薦，人會學會忽略那個標籤。**還沒打金額的消費不模擬也不排名**（全是 0 的排名等於隨機），退回原本的「還可刷／已拿滿」顯示。
