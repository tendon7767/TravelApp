...
- **選擇支付方式的選單裡的金額一律是本幣、而且一律標「約」。** 費用記的是外幣，換算用的是手填的 `trip.rate`，而發卡行入帳走它自己的結匯匯率、還可能疊海外交易手續費 —— **這個數字從一開始就是估的**，跟顯不顯示台幣無關。台幣費用配台幣計算的卡雖然沒有換算，但為那一種情形去分支，換來的是「為什麼這張沒有『約』」這種更難解釋的不一致。**例外是「還可刷」**：它維持卡片幣別，因為回饋頁看的是同一個數字，換算會讓兩邊對不起來。
