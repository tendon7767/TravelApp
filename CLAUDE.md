...
**「被別人索引內容」不該擋住「新增連住」。** 先把入住同步過來、之後再設連住，這個順序要走得通，
所以那顆按鈕看的是 `stayNightsOf` 而不是 `followersOf`。反過來，「同步自其他住宿」看的是
`followersOf` —— 已經有任何人索引我，我就不能再去索引別人（只准一層）。
