...
- `預設備註` 的每則可以是字串，或 `{ 文字, 總覽 }`；`總覽: true` 會直接顯示在行程列上。
  只給交通工具用（飛機、渡輪、巴士的「幾點從哪到哪」），那是唯一值得佔用行程列的一行。
- **「退房」在「其他」底下，不在「住宿」。** 退房是離開不是過去，設成住宿類型的話同天換飯店時
  `stayJumpTarget` 會指反。實際排出來的行程也證實了這個用法：那趟九天的退房全在「其他」，
  住宿類型底下只有「入住 X」與「住宿 X」。
- `applyTemplate` **只補空欄位，永不覆蓋**。所以帶了骨架費用之後，事後在詳細頁改類型
  就不會再套新類型的預設（`costs.length === 0` 那個條件），這是刻意的。
- **`TemplateRow` 的 `when` 決定這一列哪幾天要建**（`first`／`last`／`exceptFirst`／`exceptLast`，
  不寫就是每天）。退房是 `exceptFirst`、住宿是 `exceptLast`，出發日與回程日那兩串固定流程
  （機場捷運、飛機、機場巴士、入住／還車、買伴手禮、到家）各自標 `first` 與 `last`。
  **只有一天的旅程 first 與 last 同時成立**，退房與住宿因此都不會出現，那是對的。
- **新增旅程每天自動建的那幾筆也能沿用快選的預設值**：[dayTemplate.ts](src/lib/dayTemplate.ts) 的
  `TemplateRow` 寫 `quick: '住宿'`，`createTrip` 就用 `quickItemBy` 取那顆快選的 preset 套上去。
  所以自動建的與手動按快選建的長得一模一樣，骨架只有 JSON 那一份，不會漂移。
  目前只有 21:00 住宿那列標了；起床與三餐刻意留白（每天多三筆 0 元餐費列沒有價值）。

## 住宿的連住與資料同步

一間住宿住三晚，行程表上就是三筆記錄。**資料面刻意維持各自獨立**（每晚有自己的時間、心得、照片），
靠一個 `sourceItemId` 記號把「內容」綁在一起：主筆改一次，所有從筆立刻跟著變。
邏輯在 [stay.ts](src/lib/stay.ts)，動作在 store 的 `setStayCheckout` / `unlinkItem`。

**同步不等於連住，這是兩個欄位。** `sourceItemId` 只講「內容抄自誰」；
`stayNight` 才是「這是連住排出來的其中一晚」，只有 `setStayCheckout` 會給。
`followersOf` 是全部從筆（判斷「有沒有人索引我」用），`stayNightsOf` 才是連住的晚。
**晚數、退房日、改退房日時要刪哪幾天，一律只能數後者。**
早期版本沒分開，症狀是「共 N 晚」虛報，而且改退房日會把入住那筆連同它的心得與照片一起刪掉
—— 入住跟主筆同一天，永遠不在「入住日隔天起算」的區間裡。

**已經沒有「同步自其他住宿」了**（手動挑一筆住宿來共用內容），所以新產生的從筆一律是連住的晚。
但**這兩個欄位不能合併**：舊資料裡還有同天的手動從筆，同行者沒更新的舊版也還建得出來。
它們現在唯一的出路是那顆「解除同步」—— 走 store 才會更新 `updatedAt` 推上雲端，
手改 IndexedDB 不動 `updatedAt` 的話下次 pull 就被遠端蓋回來。
...
- **`stayNight` 要跟著 `sourceItemId` 一起收**：解除同步、主筆被刪、複製整筆，兩個都要清掉。
  留著孤兒記號的話，那一筆會在別處被當成某段連住的一晚。
...
辦理入住、寄放行李跟「回房睡覺」是不同的事，各自一筆行程，但飯店資訊只寫在**當天最後那筆**
（連住的主筆＝入住日的回房那筆）。其餘幾筆靠相關連結最上面那一列「前往住宿」跳過去，
所以它們不必再各自帶一份飯店資訊 —— 這就是「同步自其他住宿」被拿掉的原因。

規則在 `stayJumpTarget`：**同一天有兩筆以上的住宿時，除了最後一筆以外每一筆都指向最後一筆。**
只有一筆就沒有連結。退房那筆刻意不設成住宿類型（是離開不是過去），所以同天換飯店也不會指反。

- **完全不讀 `sourceItemId`。** 主筆的日期可以改、同行者的舊版還建得出同天從筆，
  「主從必然跨日」不是保證得了的性質。只看當天就怎麼亂都炸不了，最壞情況是指到一份
  一模一樣的內容，無害。
- **兩筆都沒填時間時順序不穩定**（`timeSortKey` 相同，`sort` 穩定 = 吃 items 的原始順序，
  而那個順序同步合併回來會變）。刻意不加決勝鍵：那是「該給其中一筆填時間了」的提醒，
  而且它純粹是衍生的顯示，翻面也弄不髒資料。
- **它不是 `links` 裡的一筆，編輯時整列不出現。** 沒有網址可編，混進去還會讓拖曳排序的
  index 對照錯位（`linkSort` 是「依序填回原本屬於 web 的位置」）。空狀態的判斷也要含它，
  不然一筆 web 連結都沒有的住宿會落進 `-` 分支。
- **圖示是床不是鎖鏈**，右邊那顆寫「前往住宿」（跟「開啟連結」等寬才切得齊）。
  同一顆鎖鏈會讓人以為要開瀏覽器，這一顆是 App 內換一筆。
- **跳轉走外層的 `openDetail`**（`onJump`），已經在詳細頁時它是 `replace`，歷史不累積，
  跳幾次都還是按一次返回就回得到行程列表。
...
**從筆的日期也是唯讀的，只有時間能改。** 連住那幾晚改掉會跟區間對不上
（`stayNightsOf` 數筆數、`checkoutOf` 取最後一晚 +1），入住那種手動連的挪到別天也只是奇怪。

### 沒有存 endDate，這是刻意的

退房日是「動作」不是「狀態」：按下去就把該建的建完，之後從連住那幾晚的日期反推。
...
**「被別人索引內容」不該擋住「新增連住」。** 舊資料裡可能已經有人索引我（那時還有「同步自其他住宿」），
那個順序要走得通，所以那顆按鈕看的是 `stayNightsOf` 而不是 `followersOf`。
...
`stayNight` 比 `sourceItemId` 晚一步才加，所以 [db.ts](src/store/db.ts) 有一次性遷移
（`markLegacyStayNights`）：舊的從筆若日期晚於主筆就補上記號，跟主筆同一天的維持內容同步。
只在完全沒有任何一筆帶記號時才跑，補過就不會再動。
...
| 連住 | 一間住宿住好幾晚。指定退房日後中間每一晚自動建一筆（`stayNight`，[stay.ts](src/lib/stay.ts)） |
| 主筆／從筆 | 資料的來源那一筆＝主筆；索引它的那幾筆＝從筆（`sourceItemId` 有值）。從筆不一定是連住的一晚 |
...
- **「這一筆改用這張卡會多拿多少」一律用 `marginalReward()`，不要拿金額 × 費率算。** 回饋上限會讓最後那一筆只拿到剩下的額度、單筆上限會把它砍掉一截，而推薦最需要準的時刻正好就是額度快滿的時候——上限只剩 10 元的卡用費率乘出來會寫著 96 元，然後被排到第一個。它回傳逐條規則的邊際回饋與 `covered`（通路對不對得上），畫面靠 `covered` 區分「算出來是 0」與「這條根本沒算進去」。
- **每條回饋規則各自一份交易串**（`amountsFor` / `ruleCovers`），不是所有規則吃同一份。限定通路的規則只吃得到 `CostGroup.channel` 相符的那幾筆；混在一起算的話，在 BIC CAMERA 刷的那筆會去啃掉藥妝加碼的 `rewardCap`，總回饋直接算多，而且是慢慢被吃掉的，沒有任何一個時刻看起來不對。**消費沒指定通路時，限定通路的規則一律吃不到**——寧可少算不要多算。通路存的是名字不是 id（沒有 channels 集合，名字跟著 `rules` 這個 JSON 欄位同步，同行者沒有那份清單也算得對），比對前一律走 [channels.ts](src/lib/channels.ts) 的 `channelKey` 正規化（NFKC、去空白、忽略大小寫）。
- **選擇支付方式的選單裡的回饋金額一律換算成本幣。** 跨卡才比得動 —— 有的卡用日圓算回饋、有的用台幣，直接比 ¥1 與 NT$50 會排錯。**這裡每一個數字都是估的**：費用記的是外幣，換算用手填的 `trip.rate`，而發卡行入帳走它自己的結匯匯率、還可能疊海外交易手續費。整個選單就是「現在刷下去大概會拿到多少」的試算，所以不逐個標「約」（標了也只有總額那一行有，跟底下的規則明細對不起來）。**例外是「還可刷」**：它維持卡片幣別，因為回饋頁看的是同一個數字，換算會讓兩邊對不起來。
- **額度不夠時要講出白刷了多少**（`marginalReward` 的 `earning` / `wasted`）。只給一個「比費率乘出來還小」的數字，看不出來為什麼。可賺的那截是由三樣東西夾出來的：這筆金額、回饋上限還剩多少 ÷ 費率、單筆上限 ÷ 費率。**0% 的規則與通路不符的規則都不算白刷** —— 前者本來就沒有回饋，後者顯示的是「未計入」。
- **選擇支付方式的選單是「這一筆用哪張刷」的排行榜，不是清單。** 逐條列出各規則的邊際回饋、依本幣（`inHomeCurrency`）由高到低排，各持有人分區**各自排名、各自掛一個「推薦」**——跨持有人排名會推薦到別人的卡。分區沿用回饋頁那套膠囊橫條 + 三格軌道（`SwipePager`），**最後固定接一格「其他」**（現金／其他／未設定），所以橫條永遠會出現，不必處理「只有一位持有人」的分支。預設落在「目前那張卡的持有人」→「自己的卡」→ 第一區；**現金／其他要落在「其他」區，但「未設定」不算** —— 那是新建消費的常態，每次開都掉到「其他」很煩。軌道的每一格是 `height: 100%` 的捲動層，父層沒有確定高度會整個算成 0，所以這個彈窗要 `Modal` 的 `fill`（固定七成高、`.sheetbody` 不再自己捲）。**`fill` 只給需要的那一個掛，不要去改 `variant="picker"` 本身**，否則全 App 的 picker 都會變七成高。**推薦差距不到 1 元（本幣）就不掛**，為了三毛錢掛推薦，人會學會忽略那個標籤。**底色是「目前選的那一張」不是推薦的那一張** —— 上了底色的東西本來就會被讀成「選中」，順著那個直覺走比較省事；兩者剛好同一張時只標「目前選擇」，「你已經選了」比「建議你選」有用。標籤一律排在名稱**後面**：排前面的話，有標籤與沒標籤的列名稱起點會差一整個標籤寬，整份清單就對不齊。**還沒打金額的消費不模擬也不排名**（全是 0 的排名等於隨機），退回原本的「還可刷／已拿滿」顯示。
- **通路標籤沒有獨立的集合，清單是「所有規則用過的名字」的聯集**（[channels.ts](src/lib/channels.ts) 的 `channelsOf`，跨旅程）。所以「刪除」是自動的——沒有規則引用它，下次算就不見了；也所以**新增一個標籤只能寫進草稿**（它除了那條規則之外沒有存在的依據），外層支付方式一按取消就跟著消失，放棄確認會把這件事講出來（`dirtyHint`）。**改名則是真儲存**，而且必然是全域的：只改眼前那條規則會讓同一個通路靜默分裂成兩個。`renameChannels` 掃四個地方——其他卡的 `payments.rules`、所有 `items.costGroups`、外層的支付方式草稿、以及 [drafts.ts](src/store/drafts.ts) 存著的行程草稿。**漏掉最後兩個的症狀是「改完名字自己變回去」**：草稿裡還是舊名字，下次一存就寫回來。
- **改名與新增一定要在同一次 `onChange` 裡送出**（`ChannelSheet` 的 `onSave` 一支收兩件事）。拆成兩個回呼的話，第二個是拿還沒套用改名的草稿當底重算，會把第一個寫進去的新名字整批蓋回舊的，而且完全不報錯。
- **通路清單的 a-z 排序只在編輯模式凍住。** 名稱可以就地編輯，邊打邊重排的話手指底下那一列會跳走；但離開編輯模式要放掉，不然剛改好的名字還顯示成舊的。
