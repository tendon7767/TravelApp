...
└── 旅遊資料/
    ├── 2026 秋季 瀨戶內海9日遊/
    │   └── 2026 秋季 瀨戶內海9日遊.gsheet
    └── 2026 夏季北海道12天/
        └── 2026 夏季北海道12天.gsheet
```

在 App 裡改旅程名稱，同步時資料夾與試算表會一起改名，不會在硬碟裡變成一堆認不出來的舊名字。

### 換一個上層資料夾（選用）

預設上層是根目錄的「旅遊資料」。想放到自己既有的資料夾：

1. 在雲端硬碟打開那個資料夾，複製網址（長得像 `https://drive.google.com/drive/folders/1z-A6PA...`）
2. App → ⚙ →「旅程資料夾的存放位置」貼上 → 按「確認並儲存」
3. 會顯示「將建立於：我的雲端硬碟 / 旅遊 / 2026」讓你確認沒放錯地方

只影響**之後**建立的旅程。已經建好的想搬就在雲端硬碟裡直接拖，App 是靠檔案 ID 找的，搬了照樣連得上。
