...
預設會建在雲端硬碟根目錄的 `TravelApp` 資料夾。想放到自己既有的資料夾：

1. 在雲端硬碟打開那個資料夾，複製網址（長得像 `https://drive.google.com/drive/folders/1z-A6PA...`）
2. App → ⚙ →「試算表存放位置」貼上 → 按「確認並儲存」
3. 會顯示「將建立於：我的雲端硬碟 / 旅遊 / 2026」讓你確認沒放錯地方

只影響**之後**建立的旅程，已經建好的試算表不會搬動 —— 要搬就在雲端硬碟裡直接拖，App 是靠 ID 找檔案的，搬了照樣連得上。
